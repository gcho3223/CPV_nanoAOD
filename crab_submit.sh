...
for sample in "${samples[@]}";
    ...
    crab_dir="./Crab_jobs/2018/${sample_type}/${sample}"
    if [ -d "$crab_dir" ]; then
        if [ "$1" == "submit" ]; then
            cd "${crab_dir}"
            echo "Current directory: $(pwd)"
            echo crab submit -c "crab_config.py"
            crab submit -c "crab_config.py"
            cd -
        elif [ "$1" == "resubmit" ]; then
            echo crab resubmit -d "${crab_dir}/crab_${sample}"
            crab resubmit -d "${crab_dir}/crab_${sample}"
        fi
    else
